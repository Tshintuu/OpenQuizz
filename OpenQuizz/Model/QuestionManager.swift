...
class QuestionManager {
    ...
    private func getQuestionFrom(parsedData: [String: Any]) -> Question {
        if let title = parsedData["question"] as? String,
            ...
            let category = parsedData["category"] as? String,
            let difficulty = parsedData["difficulty"] as? String {
            
            return Question(title: String(htmlEncodedString: title)!, isCorrect: (answer == "True"),
                            category: category,
                difficulty: setDifficulty(level: difficulty))
        }
        return Question()
    }
    
    private func setDifficulty(level difficulty: String) -> Question.Difficulty {
        switch difficulty {
        case "easy":
            return .easy
        case "medium":
